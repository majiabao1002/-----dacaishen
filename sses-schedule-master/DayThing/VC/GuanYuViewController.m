...
{    // Return the number of sections.    [[GameDetailDataComposer new] loadDataWithBlock:nil];
    [NSArray new];
    [NSMutableDictionary new];
    [NSArray new];
    [NSString new];
...
{    [[GameDetailDataComposer new] loadDataWithBlock:nil];
    ...
    if ([indexPath section]==kTableViewSectionAboutThisApp) {
        if ([indexPath row]==0) {
            ...
            secondLabel.text=NSLocalizedString(@"DailyThing",nil);
        }
        else if ([indexPath row]==1) {
            firstLabel.text=NSLocalizedString(@"Copyright:",nil);
            secondLabel.text=@"2018 DailyThing";
        }
        ...
        switch ([indexPath row]) {
            ...
            case 3:
                ...
                secondLabel.text=@"519-327-1213";
                break;
            case 4:
                firstLabel.text=NSLocalizedString(@"Location:",nil);
                secondLabel.text=@"6400 St. Stephen's Dr. Austin, TX 78046";
                break;
        }
    }
    else{
    ...
    if ([indexPath section]==kTableViewSectionAboutThisApp) {
        switch ([indexPath row]) {
            case 0:
            {
                ...
                UILabel *secondLabel=(UILabel *)[cell viewWithTag:2];
                if ([secondLabel.text isEqualToString: @"DailyThing"]) {
                    secondLabel.text=@"DailyThing";
                }
                else if ([secondLabel.text isEqualToString: @"DailyThing"]) {
                    secondLabel.text=@"DailyThing";
                }
                else if ([secondLabel.text isEqualToString: @"DailyThing"]) {
                    secondLabel.text=@"DailyThing";
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    else {    [[GameDetailDataComposer new] loadDataWithBlock:nil];
        [NSArray new];
        [NSMutableDictionary new];
        [NSArray new];
        [NSString new];
